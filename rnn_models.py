...
class IVS_RNN(torch.nn.Module):
    def __init__(self, features, hidden_size, output_timesteps, num_layers, num_maturities, num_moneynesses, batch_first=True, dropout=0.0):
...
class IVS_LSTM(IVS_RNN):
    def __init__(self, features, hidden_size, output_timesteps, num_layers, num_maturities, num_moneynesses, batch_first=True, dropout=0.0):
        super(IVS_LSTM, self).__init__(features, hidden_size, output_timesteps, num_layers, num_maturities, num_moneynesses, batch_first, dropout)
...
class IVS_GRU(IVS_RNN):
    def __init__(self, features, hidden_size, output_timesteps, num_layers, num_maturities, num_moneynesses, batch_first=True, dropout=0.0):
        super(IVS_GRU, self).__init__(features, hidden_size, output_timesteps, num_layers, num_maturities, num_moneynesses, batch_first, dropout)
...
class RNNForecastModel:
    ...
    def __init__(self, ticker: str, start: str, end: str, split_date: str, maturities: list[int], moneynesses: np.array, rnn_hidden_size: int,
                rnn_num_layers: int, ivs_rnn_type: Type[IVS_RNN], output_timesteps: int=1, rnn_dropout: float=0.0, loss_fn: torch.nn.modules.loss._Loss=MSELoss()):
        """
        ...
        # Constructing model
        self.rnn_model = ivs_rnn_type(1, rnn_hidden_size, output_timesteps, rnn_num_layers, len(maturities), len(moneynesses), dropout=rnn_dropout).to(self.device)
    ...
    def train(self, epochs: int, batch_size: int, input_timesteps: int, dynamic_load: bool=False, quiet: bool=False) -> list[float]:
        ...
        """
        if dynamic_load:
            batches = lambda x: self.model_data_loader.train_batch_gen(input_timesteps, self.output_timesteps, batch_size)
        else:
            # Loading training data
            X_train, y_train, bs_info_train = self.model_data_loader.train_data(input_timesteps, self.output_timesteps)
    ...
    def options_pricing(self, input_timesteps: int):
        ...
        X, y, bs_info = self.model_data_loader.test_data(input_timesteps, self.output_timesteps, forecast=True)
        ...
        for i, dte in enumerate(self.maturities):
            for j, moneyness in enumerate(self.moneynesses):
                iter_options = pd.DataFrame(
                    {
                        "Call Price": call_prices[:, 0, i, j, 0].cpu().detach().numpy(),
                        "Put Price": put_prices[:, 0, i, j, 0].cpu().detach().numpy(),
                        "Call IV": output[:, 0, i, j, 0].cpu().detach().numpy(),
                        "Put IV": output[:, 0, i, j, 1].cpu().detach().numpy(),
                        "Dividend Yield": bs_info["q"][:, 0, i, j, 0].cpu().detach().numpy(),
                        "Annualized Rate": bs_info["r"][:, 0, i, j, 0].cpu().detach().numpy()+1,
                        "Underlying Price": bs_info["S"][:, 0, i, j, 0].cpu().detach().numpy(),
                        "Strike Price": bs_info["K"][:, 0, i, j, 0].cpu().detach().numpy(),
                        "Tau": bs_info["t"][:, 0, i, j, 0].cpu().detach().numpy(),
                        "DTE": dte,
                        "Realized Call Value": y[:, 0, i, j, 0].cpu().detach().numpy(),
                        "Realized Put Value": y[:, 0, i, j, 1].cpu().detach().numpy(),
                        "Moneyness": moneyness,
                        "Date": date[:, 0]
                    }
                )
