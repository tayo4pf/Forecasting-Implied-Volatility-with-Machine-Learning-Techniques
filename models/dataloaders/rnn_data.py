...
import numpy as np
import pandas as pd
from utils.distributions import get_realized_option_value, get_returns
...
class RNNDataLoader:
    ...
    def train_data(self, input_timesteps: int) -> tuple[np.array, np.array, dict]:
        """
        ...
        for i in range(len(self.data["Log Returns"][:self.split_date])-input_timesteps-1):
        ...
        for i in range(len(self.data["Log Returns"][:self.split_date])-input_timesteps-1):
            ...
            y = self.train_y[i+1:i+input_timesteps+1, :, :, :]
            y_S = self.train_S[i+1:i+input_timesteps+1, :, :]
            y_K = self.train_K[i+1:i+input_timesteps+1, :, :]
            y_t = self.train_t[i+1:i+input_timesteps+1, :, :]
            y_r = self.train_r[i+1:i+input_timesteps+1, :, :]
            y_q = self.train_q[i+1:i+input_timesteps+1, :, :]
    ...
    def test_data(self, input_timesteps: int, forecast: bool=False) -> tuple[np.array, np.array, dict]:
        """
        ...
        for i in range(len(self.data["Log Returns"][self.split_date:])-input_timesteps-1):
            ...
            y = self.test_y[i+1:i+input_timesteps+1, :, :, :]
            y_S = self.test_S[i+1:i+input_timesteps+1, :, :]
            y_K = self.test_K[i+1:i+input_timesteps+1, :, :]
            y_t = self.test_t[i+1:i+input_timesteps+1, :, :]
            y_r = self.test_r[i+1:i+input_timesteps+1, :, :]
            y_q = self.test_q[i+1:i+input_timesteps+1, :, :]
            if forecast:
                dates = np.array(self.data["Log Returns"][self.split_date:][i+1:i+input_timesteps+1].index)
                date.append(dates)
    ...
    def train_batch_gen(self, input_timesteps: int, output_timesteps: int, batch_size: int):
        ...
        for i in range(len(self.data["Log Returns"][:self.split_date])-input_timesteps-1):
            ...
            if batch_count == batch_size or i == len(self.data["Log Returns"][:self.split_date])-input_timesteps-2:
                batch_count = 0
    ...
    def test_batch_gen(self, input_timesteps: int, batch_size: int):
        """
